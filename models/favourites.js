...
exports.selectAllFavourites = async (username, sort_by = "popularity", order = "desc") => {
  ...
  const mainQuery = format(
    `SELECT spots.spot_id AS spot_id, spots.name AS name, ST_X(spots.location) AS latitude, ST_Y(spots.location) AS longitude, spots.opening_time AS opening_time, 
...
exports.insertFavourite = async (username, spot_id) => {
  const {
    rows: [row],
  } = db.query("INSERT INTO favourites (spot_id, username) VALUES ($1,$2) RETURNING *;", [
    spot_id,
    username,
  ]);
